...
void SepGame::start_game()
{
    ...
    while (window.isOpen())
    {
        ...
        {
            clock.restart();
            if (Obstacles.size() <= ASTEROID_LIMIT)
        ...
        for (auto asteroid: Obstacles)
        {
            ...
            if (fabs(PlayerPos.x - AsteroidPos.x) < 0.8 * (PlayerDims.width + BoudlderDims.width)/2 &&
                fabs(PlayerPos.y - AsteroidPos.y) < 0.8 * (PlayerDims.height + BoudlderDims.height)/2)
            {
                isGameOver = true;
            }
        }
