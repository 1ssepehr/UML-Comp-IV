...
void Universe::load() {
  ...
  while (isOpen()) {
    ...
    step(delta_t);
    if (curTime > endTime) delta_t = 0;

    // Printing time
    auto days = curTime / SECS_IN_DAY;
    std::stringstream timer_text;
    timer_text << "Timer: " << std::fixed << std::setprecision(2) << days
               << " days ";
    timer_text << "(" << std::fixed << std::setprecision(1)
               << 100 * (curTime / endTime) << "%)" << std::endl;
    timer_text << "[=" << std::fixed << std::setprecision(0) << curTime
               << " seconds]";
...
std::istream &operator>>(std::istream &in, Universe &universe) {
  ...
  auto bgScale =
      universe.getSize().x / universe.bgSprite.getLocalBounds().width;
...
std::ostream &operator<<(std::ostream &out, Universe &universe) {
  ...
  for (auto &body : universe.bodyVec) out << *body << std::endl;
  return out;
}

// This step function uses the Runge-Kutta method of 4th order for estimating
// the position of the planets after a change of `h` in time. This is more
// calculation-heavy, but results in much much approximation error, and the
// trade-off makes the simulation more accurate overall.
void Universe::step(double h) {
  curTime += h;

  std::vector<Point> net_force(N);
  std::vector<double> mass(N);
  for (unsigned i = 0; i < N; i++) mass[i] = bodyVec[i]->mass;
  std::vector<Point> position(N);

  // Function to calculate forces based on given position (position) and mass
  // (mass). Updates the content of `std::vector<Point> net_force` with the new
  // calculated forces. We use this function multiple times for approximations
  // of Runge-Kutta method.
  auto calculate_forces = [](const std::vector<Point> &position,
    ...
    for (unsigned i = 0; i < N; i++) {
      ...
      for (unsigned j = 0; j < N; j++) {
        if (i == j) continue;
        Point displacement{position[j].x - position[i].x,
                           position[j].y - position[i].y};
        auto displacement_squared = (displacement.x * displacement.x) +
  ...
  for (unsigned i = 0; i < N; i++) position[i] = bodyVec[i]->r;
...
void Universe::loadArtifacts() {
  if (!timer_font.loadFromFile(FONT_PATH) ||
      !bgMusic.openFromFile(MUSIC_PATH) || !bgTexture.loadFromFile(BG_PATH))
